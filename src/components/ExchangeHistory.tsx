...
const ExchangeHistory = () => {
  return (
    <div className="bg-white rounded-xl shadow p-4">
      <h3 className="text-base font-bold text-gray-800 mb-3">Exchange History</h3>
      
      <div className="space-y-3">
        <div className="flex justify-between items-center">
          <p className="text-sm text-gray-600">Recent Exchanges</p>
        ...
        {exchangeHistory.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="mt-2 text-sm">No exchange history yet</p>
            <p className="text-xs">Complete your first exchange to see it here</p>
          </div>
        ) : (
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {exchangeHistory.map((exchange) => (
              <div key={exchange.id} className="border border-gray-200 rounded-lg p-3 hover:bg-gray-50 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center space-x-2">
                    <div className="flex items-center space-x-1">
                      <span className="text-sm font-medium">{exchange.fromAmount.toLocaleString()}</span>
                    ...
                    <div className="flex items-center space-x-1">
                      <span className="text-sm font-medium">{exchange.toAmount.toLocaleString()}</span>
