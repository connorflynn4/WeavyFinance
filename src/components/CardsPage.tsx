    ...
    <div className="max-w-5xl mx-auto px-2 sm:px-0">
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1">Manage your physical and virtual cards, limits, and security.</p>
        </div>
        <button onClick={addVirtualCard} className="inline-flex items-center gap-2 rounded-lg bg-teal-600 text-white px-3 py-2 text-xs sm:text-sm font-medium hover:bg-teal-700 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow p-3 sm:p-4">
          <div className="bg-gradient-to-r from-teal-500 to-teal-600 rounded-lg p-3 sm:p-4 text-white mb-4">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-2">
              <div>
                <p className="text-xs/4 opacity-90">{cards[0]?.holder}</p>
                <p className="text-base sm:text-lg font-bold mt-1">**** **** **** {cards[0]?.last4}</p>
              </div>
            ...
            {cards.map(card => (
              <div key={card.id} className="flex items-center justify-between border border-gray-200 rounded-lg p-3 hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-md bg-gray-100 flex items-center justify-center text-gray-500">
                    <CreditCard className="w-4 h-4 sm:w-5 sm:h-5" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="text-xs sm:text-sm font-medium text-gray-900">**** **** **** {card.last4}</p>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleShow(card.id)} className="rounded-md border border-gray-300 px-2 py-1 text-xs text-gray-600 hover:border-gray-400">
                    {showNumbers[card.id] ? <EyeOff className="w-3 h-3 sm:w-4 sm:h-4" /> : <Eye className="w-3 h-3 sm:w-4 sm:h-4" />}
                  </button>
                  <button onClick={() => toggleFreeze(card.id)} className="rounded-md border border-gray-300 px-2 py-1 text-xs text-gray-600 hover:border-gray-400">
                    {card.status === 'Active' ? <Lock className="w-3 h-3 sm:w-4 sm:h-4" /> : <Unlock className="w-3 h-3 sm:w-4 sm:h-4" />}
                  </button>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white rounded-xl shadow p-3 sm:p-4 space-y-4">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center justify-between">
              <p className="text-xs sm:text-sm font-medium">Disabled</p>
              <label className="inline-flex items-center cursor-pointer">
